...
# Só define o valor inicial se a chave não existir. Isso evita o reset para "Maio".
...
# Usamos a key diretamente para que o Streamlit gerencie o estado.
st.sidebar.selectbox("Mês", meses_nomes, key='mes_sel')
st.sidebar.selectbox("Ano", anos, key='ano_sel')

# Lemos os valores atualizados do session_state
...
# TÍTULO (O texto em amarelo na sua imagem agora fica sincronizado)
...
if st.session_state['df_bruto'] is not None:
    try:
        ...
        # Filtro de dados baseado no mês selecionado
        ...
        if not df_filtrada.empty:
            ...
            df_conferencia['CNPJ Contraparte'] = df_conferencia[col_boleta].map(df_lookup[df_base.columns

...
st.sidebar.subheader("Bases Cliq CCEE (Formatos CSV)")
...
# 4. PROCESSAMENTO DAS BASES DE APOIO (Ajustado para CSV)
def carregar_cliq_csv(arquivo):
    if arquivo:
        try:
            # Tenta ler com separador ';' que é o padrão CCEE/Brasil, se falhar tenta ','
            try:
                df = pd.read_csv(arquivo, sep=';', encoding='latin-1')
                if df.shape[1] < 2: raise Exception("Tentando outro separador")
            except:
                arquivo.seek(0)
                df = pd.read_csv(arquivo, sep=',', encoding='utf-8')
            
            # Col D (índice 3) é a Boleta
...
if arquivo_subido:
    try:
        ...
        if df_filtrada.empty:
            ...
            # Lógica Cliq CCEE (Contrato Atual)
            def buscar_cliq_ccee(row):
                ...
                parte_val = str(orig.iloc[62]).upper() # Baseado na BK
                ...
                for db in bases:
                    if db is not None and boleta in db.index:
                        ...
                        # CSVs mantêm a ordem das colunas
            ...
            # Ordem final (ID Cliq CCEE por último)
            ordem = [
